        ...
        if variable == "Vehicle Volume":
            ...
            if nb_col and sb_col:
                ...
                if chart_type == "Line":
                    ...
                    def get_existing_cycle_length(volume):
                        # Placeholder: customize if your system logic is different
                        return get_hourly_cycle_length(volume)


                    # --- DATA CLEANING FOR YOUR FORMAT ---
                    def prepare_hourly_df(df):
                        cols = df.columns.tolist()
                        df = df.rename(columns={cols[1]: 'Northbound', cols[2]: 'Southbound'})
                    ...
                    # --- SIDEBAR SELECTORS ---
                    st.sidebar.header('Traffic Volume Dashboard Filters')
                    ...
                    show_cycle_length = st.sidebar.toggle("Show Cycle Length Recommendations", value=True)

                    # --- MAIN CONTENT ---
                    st.title('Cycle Length Recommendations - Hourly Analysis')

                    uploaded = st.file_uploader("Upload Hourly Volume CSV", type="csv")

                    if uploaded is not None:
                        df = pd.read_csv(uploaded)
                        df = prepare_hourly_df(df)
                        ...
                        hours = period_map.get(period_key, list(range(24)))
                        filtered_df = df[df['Hour'].isin(hours)]

                        # Choose correct volume column
                        ...
                        if show_cycle_length:
                            st.markdown("#### Hourly Cycle Length Table")
                            ...
                            st.info("Cycle length recommendations are hidden. Enable toggle to view analysis.")
                    else:
                        st.info(
                            "Upload a CSV to begin analysis.\n\nFormat: [Time, <date> Northbound, <date> Southbound]")
